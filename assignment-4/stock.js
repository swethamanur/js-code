...
var tickerObject = {};
    var output = {};
    var sum = 0;
    for(var k = 0 ; k < ticker.length; k = k + 2){
         tickerObject[ticker[k]] = ticker[k+1]; /*assigning every element to an object*/
    }
    console.log(tickerObject);
    for(var i =0; i < portfolio.length; i++){
        if(tickerObject.hasOwnProperty(portfolio[i][0])){
             output[portfolio[i][0]] = portfolio[i][1] * tickerObject[portfolio[i][0]];
            sum = sum +  output[portfolio[i][0]];
        }else{
            output[portfolio[i][0]] = 'company does not exist';
        }
    }
    output['total'] = sum; //assigning final total to the object
